...
const SForth = Object.freeze({
    version: "1.0.0",
});

const e_flags = {
    "-v": () => {
        console.log("SynctLabs Forth Interpreter", SForth.version);
    },
}

for (let i = 2; i < process.argv.length; i++) {
    if (process.argv[i][0] == "-" && e_flags[process.argv[i]]) {
        e_flags[process.argv[i]]();
        process.exit(0);
    }
}

let code = "";
let main = fs.readFileSync(process.argv[2], { encoding: "utf-8" });
let line = 0;
...
let tokens = {
    "+": () => {
        if (stack.length >= 2) {
            ...
            stack.pop();
            stack.push(last);
            if (!flags.includes("-noexit")) console.log("Ok");
        } else {
    ...
    ".S": () => {
        if (stack.length >= 1) {
            let exit = "{";
            stack.forEach((item, id) => {
                exit += `${item} `;
            });
            process.stdout.write(String(exit.trim().replaceAll(" ", ", ") + "}"));
        } else {
            console.log("Error >> Stack need 1 or more values for printing.");
        }
    },
    ...
    if (Number.isInteger(parseInt(item))) {
        ...
        if (item.trim() != "") {
            if (tokens[item]) {
                ...
                if (flags.includes("-vanish")) console.log(">", item);
            } else {
                if (item[0] === "-") {
                    flags.push(item);
                } else {
